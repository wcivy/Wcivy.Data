    ...
    public class QueryBuilder<T> : IQueryBuilder<T> where T : class
    {
        ...
        private sealed class PredicateConverter
        {
            ...
            private IPredicate ParseCallQueryFunction(MethodCallExpression expression)
            {
                ...
                {
                    var memberExpression = SimplifyExpression(expression.Arguments[0]) as MemberExpression;
                    var patternExpression = expression.Arguments[1] as ConstantExpression;
            ...
            private IPredicate ParseCallEquals(MethodCallExpression expression)
            {
                ...
                {
                    throw new NotImplementedException();
                    //object objectComparison = expression.Arguments[1].NodeType == ExpressionType.Constant ?
                    //((ConstantExpression)expression.Arguments[1]).Value :
                    //InvokeExpression(expression.Arguments[1]);
                    //if (objectComparison.GetType() == typeof(StringComparison))
                    //{
                    //    var stringComparison = (StringComparison)objectComparison;
